...
COMPANIES = [
    ...
    {"name": "Just Eat",             "category": "BD & Sales"},
    {"name": "Deliveroo",            "category": "BD & Sales"},
]
...
def scrape_company(company_name, category):
    try:
        jobs = scrape_jobs(
            ...
            google_search_term=f'{company_name} jobs United Kingdom',
        ...
        for _, job in jobs.iterrows():
            ...
            if company_name.lower().split()[0] not in company_col:
                continue
            results.append({
                ...
                "job_type":   str(job.get("job_type", "")).capitalize() or "Experienced",
                "apply_link": str(job.get("job_url", "")),
                "date_added": datetime.now().strftime("%Y-%m-%d"),
                "status":     "Active",
                "source":     str(job.get("site", ""))
            })
        print(f"  OK {company_name}: {len(results)} jobs")
        return results
    except Exception as e:
        print(f"  FAIL {company_name}: {e}")
        return []
    ...
    try:
        ...
        try:
            for row in sheet.get_all_values()[1:]:
                if len(row) >= 6: existing.add(row[5])
        except: pass
        added = 0
        for job in all_jobs:
            if job["apply_link"] in existing: continue
            sheet.append_row([job["job_title"],job["company"],job["category"],
                job["location"],job["job_type"],job["apply_link"],
                job["date_added"],job["status"],job["source"]])
            existing.add(job["apply_link"])
            added += 1
        print(f"Added {added} new jobs to Google Sheets")
    except Exception as e:
        print(f"Google Sheets error: {e}")

def run():
    print(f"RichardUKJob Scraper - {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    all_jobs = []
    for i, company in enumerate(COMPANIES):
        print(f"[{i+1}/{len(COMPANIES)}] {company['name']}...")w h
    ...
    with open("jobs_output.csv","w",newline="",encoding="utf-8") as f:
        w = csv.DictWriter(f,fieldnames=["job_title","company","category","location","job_type","apply_link","date_added","status","source"])
        w.writeheader()
        w.writerows(unique)
    if GOOGLE_SHEET_ID:
